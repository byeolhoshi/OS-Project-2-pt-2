...
typedef struct Restaurant { // make sure theres no race conditions, sem_wait and sem_post
    Order* orders;
    int current_size; //coutner
    int max_size; //counter
    int next_order_number; //counter
    int orders_handled; //counter
	  int expected_num_orders; //counter
    pthread_mutex_t mutex; //deallocate this
    pthread_cond_t can_add_orders, can_get_orders; // customers produce, cooks consume [get_orders]
    //when order leaves queue send signal that customers can add order, vice versa
} BENSCHILLIBOWL;
